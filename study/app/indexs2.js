...
// 모듈가져오기
import {signIn, signUp} from "./service/auth-service.js"; //로그인, 회원가입
import {createMemo, loadMemos} from "./service/memo-service.js";  //메모 작성, 불러오기
import {prompt} from "./util.js";   //프롬프트 함수로 콘솔 입력 처리
...
while (true) {
  ...
  while (true) {
    ...
    if (choice === 1) {
      ...
      for (let i = 0; i < memos.length; i = i + 1) { //현재 사용자의 메모 목록 가져와서 출력
        console.log(`제목: ${memos[i].title}, 컨텐츠: ${memos[i].content}`);
      }
    } else if (choice === 3) {
      me = null; // me를 null로 변경, 로그인 상태 해제
      break;
    } else if (choice === 4) {
      process.exit(0);
    } else {
